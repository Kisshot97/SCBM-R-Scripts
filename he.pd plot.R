...
ggsave(
  filename = paste0("D:/personal/work/test326/ACTGout/pic/", "VIMPx", ".eps"), # 修改扩展名为 .eps
  plot = VIMP_PLOT, # 图像对象
  width = 12, # 图像宽度
  height = 8, # 图像高度
...
#######################################PD###################################################
  ...
  registerDoParallel(cl)
  for (i in 1:ncol(X)) {
    print(paste0("foreach=",i))
    XX <- X
    XX[, i] <- sort(X[, i]) 
    uniXX=unique(XX[, i])
    pdout[[i]] <- foreach(k = 1:length(uniXX), .combine = 'c', .packages = 'grpreg') %dopar% {######
      
      XX[, i] <- uniXX[k]
      XX_test1 <- numeric()
      for (j in 1:nrow(XX)) {
        xin <- matrix(unlist(rep(XX[j, ], nrow(cut0))), nrow = nrow(cut0), byrow = TRUE) - cut0
        dirin <- xin * dir0
        a02 <- which(dir0 == 2)
        dirin[which(dirin == 0)] <- 1
        dirin[dirin < 0] <- 0
  ...
  # Initialize an empty list to store plots for each column
  plot_list <- list()
  
  # Loop over each column in X to prepare data and create individual plots
  for (i in 1:ncol(X)) {
    XX <- X  # Copy X to avoid modifying the original
    XX[, i] <- sort(X[, i]) # Sort the i-th column
    uniXX <- unique(XX[, i]) # Get unique sorted values of the i-th column
    
    # Get the corresponding HTE values from pdout[[i]]
    HTE <- pdout[[i]]
    ...
    col_data <- data.frame(
      X = uniXX,
      HTE = HTE,
      Group = paste0("Column_", i)
    )
    
    # Create a point-line plot for the current column with the column name as the x-axis label
    plot_list[[i]] <- ggplot(col_data, aes(x = X, y = HTE, color = Group)) +
      geom_line(color = "#B7C9F2", linewidth = 1.2) +
      geom_point(color = "#0B60B0") +
      ylab("HTE") +
      xlab(colnames(X)[i]) +  # Use column name as x-axis label
      theme(
        legend.position = "none",
  ...
  grid_plot <- do.call(grid.arrange, c(plot_list, ncol = 6))  # Adjust `ncol` as needed
  
  # Optionally save the grid plot
  ggsave(
    filename = paste0("D:/personal/work/test326/ACTGout/pic/", "He_PDPLOT", ".eps"), # 修改扩展名为 .eps
    plot = grid_plot, # 图像对象
    width = 12, # 图像宽度
    height = 6, # 图像高度
    device = cairo_ps # 使用 Cairo PostScript 设备
