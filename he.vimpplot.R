...
for (r in 1) {
  ...
  bcd <- foreach(i = 1:nboots, .packages = c("earth")) %dopar% {
...
q0=list()
for (i in 1:nboots) {
  if (is.character(bcd[[i]])==TRUE) {
    vimp_sorted=rep(999,12)
    names(vimp_sorted)=colnames(X)
    counter1=counter1+1
  }else{   
...
output[pc+1,]=vimp_sorted
}
output[1,]=colnames(X)
write.csv(output,paste0("D:/personal/work/test326/ACTGout/", "VIMP.",pc2,".csv"))
}
